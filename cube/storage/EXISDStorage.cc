...
void EXISDStorage::pollAdd() {
    ...
    bool isV2;
    {
        ...
        if (!recvR7(device, r1, commandVersion, vhsEcho, checkPatternEcho)) {
            WARN("Failed to receive R7 for CMD8");
            return;
        }
    ...
    do {
        ...
        if (isMMC) {
            ...
            if (!sendCommand(device, Command::SendOpCond, argument)) {
                WARN("Failed to send CMD1");
                return;
            }

            if (!recvR1(device, r1)) {
                WARN("Failed to receive R1 for CMD1");
                return;
            }
            ...
            if (!sendCommand(device, Command::AppCmd, argument)) {
                WARN("Failed to send CMD55");
                return;
            }

            if (!recvR1(device, r1)) {
                WARN("Failed to receive R1 for CMD55");
                return;
            }
            ...
            if (!sendCommand(device, AppCommand::SDSendOpCond, argument)) {
                WARN("Failed to send ACMD41");
                return;
            }

            if (!recvR1(device, r1)) {
                WARN("Failed to receive R1 for ACMD41");
                return;
            }
    ...
    if (isV2) {
        ...
        if (!sendCommand(device, Command::ReadOCR, argument)) {
            WARN("Failed to send CMD58");
            return;
        }

        u32 ocr;
        if (!recvR3(device, r1, ocr)) {
            WARN("Failed to receive R3 for CMD58");
            return;
        }
...
bool EXISDStorage::sendCommandAndRecvR1(EXI::Device &device, u8 command, u32 argument, u8 r1Mask) {
    if (!sendCommand(device, command, argument)) {
        WARN("Failed to send CMD%u", command);
        return false;
    }

    u8 r1;
    if (!recvR1(device, r1)) {
        WARN("Failed to receive R1 for CMD%u", command);
        return false;
    }

    if (r1 & r1Mask) {
        WARN("CMD%u error %02x", command, r1);
        return false;
    }
